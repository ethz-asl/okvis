...
inline static Eigen::Matrix<PrimType_, 3, 3> skewVector(const Eigen::Matrix<PrimType_, 3, 1>& vec) {
...
class NormalVectorElement: public ElementBase<NormalVectorElement,NormalVectorElement,2> {
 public:
  ...
  NormalVectorElement(const Eigen::Vector3d& vec): e_x(1,0,0), e_y(0,1,0), e_z(0,0,1){
    setFromVector(vec);
  }
  NormalVectorElement(const Eigen::Quaterniond& q): e_x(1,0,0), e_y(0,1,0), e_z(0,0,1){
    q_ = q;
  }
  ...
  static Eigen::Vector3d getRotationFromTwoNormals(
      const Eigen::Vector3d& a, const Eigen::Vector3d& b, const Eigen::Vector3d& a_perp){
  ...
  static Eigen::Matrix3d getRotationFromTwoNormalsJac(
    ...
    if(crossNorm<1e-6){
      ...
      return -1/crossNorm*(crossNormalized*b.transpose()-(crossNormalizedSqew*crossNormalizedSqew*skewVector(b)*angle));
    }
  }
...
public:
    ...
    AngleElement(double cosTheta) {
        setFromCosine(cosTheta);
    }
    ...
    virtual const mtGet& get(unsigned int i = 0) const {
      return *this;
    }
    const double* data() const {
        return cs_.data();
